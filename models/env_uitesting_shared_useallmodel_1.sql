...
AllStunningOne AS (
  ...
  SELECT 
    ...
    x -> x % 2 == 0) or ilike('Spark', '_Park') or regexp_like('%SystemDrive%\\Users\\John', '%SystemDrive%\\\\Users.*') or ('%SystemDrive%\\Users\\John' REGEXP '%SystemDrive%\\\\Users.*') or regexp('%SystemDrive%\\Users\\John', '%SystemDrive%\\\\Users.*') or (1 IN (2, 3, 4)) or (isnan(CAST('NaN' AS double))) or isnotnull(1) or isnull(1) or array_contains(json_object_keys('{"key": "value"}'), 'key1') or like('Spark', '_park') or map_contains_key(map(1, 'a', 2, 'b'), 1) or map_contains_key(map_concat(map(1, 'a', 2, 'b'), map(3, 'c')), 4) or map_contains_key(map_filter(map(1, 0, 2, 2, 3, -1), 
